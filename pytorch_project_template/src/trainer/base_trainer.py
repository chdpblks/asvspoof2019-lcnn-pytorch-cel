...
class BaseTrainer:
    ...
    def _log_scalars(self, metric_tracker: MetricTracker):
        ...
        for metric_name in metric_tracker.keys():
            val = metric_tracker.avg(metric_name)
            if hasattr(val, "item"):
                val = val.item()
            self.writer.add_scalar(f"{metric_name}", float(val))
        ...
        if not (only_best and save_best):
            try:
                ...
                if Path(filename).exists():
                    self.logger.info(f"Checkpoint saved successfully: {filename}")
                else:
                    self.logger.error(f"Checkpoint file not created: {filename}")
        ...
        if save_best:
            ...
            try:
                ...
                if Path(best_path).exists():
                    self.logger.info(f"Best checkpoint saved successfully: {best_path}")
                else:
                    self.logger.error(f"Best checkpoint file not created: {best_path}")
                if self.config.writer.log_checkpoints:
                    self.writer.add_checkpoint(best_path, str(self.checkpoint_dir.parent))
            except Exception as e:
                self.logger.error(f"Failed to save best checkpoint {best_path}: {e}")
